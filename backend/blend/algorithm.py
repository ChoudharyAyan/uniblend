...
def calculate_blend(
    ...
    artist_jaccard = len(common_artists_set) / len(all_artists_set) if all_artists_set else 0

    set_t1 = set(u1.top_tracks)
    set_t2 = set(u2.top_tracks)
    common_tracks_set = set_t1 & set_t2
    all_tracks_set = set_t1 | set_t2
    track_jaccard = len(common_tracks_set) / len(all_tracks_set) if all_tracks_set else 0
